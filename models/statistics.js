...
exports.monitorStatistics = () => {
  ...
  async function dataTypeMonitor(dataType, mst) {
  ...
  function statistics(time) {
    ...
    const mst = model.startTime;
    const {
      elecMonitorToData,
      elecMonitorYesData
    } = dataTypeMonitor('electricity', mst);

    const {
      countMonitorToData,
      countMonitorYesData
    } = dataTypeMonitor('counter', mst);
    const {
      powerMonitorToData,
      powerMonitorYesData
    } = dataTypeMonitor('power', mst);

    // eslint-disable-next-line max-len
    if (elecMonitorToData || elecMonitorYesData || countMonitorToData || countMonitorYesData || powerMonitorToData || powerMonitorYesData) {
